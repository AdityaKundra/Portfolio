...
const Modal = ({ isOpen, isClose }) => {
    ...
    return (
        <div className='absolute top-0 left-0 modal min-h-screen min-w-screen z-10 flex justify-center items-center shadow-2xl ' style={{ boxShadow: 'rgba(0, 0, 0, 0.15) 0px 10px 30px 0px' }} onClick={isClose}>{/* bg-[#00000012] */}
            <div className='h-[350px] w-[700px] bg-white flex rounded-xl z-20' onClick={(e) => e.stopPropagation()}>
                    ...
                    <ul className='text-start'>
                        {
                            projectList.map((elm,index)=>(
                                elm.type!=='Build' && (
                                    <li className='text-xs text-[#272727] py-1 cursor-pointer font-medium' key={index}>{elm.name.replaceAll('_',' ')}</li>
                                )
                            ))
                        }
                    ...
                    <ul className='text-start'>
                        {
                            Object.entries(socialLinks).map(([platform,link],index)=>(
                                <li className='text-xs text-[#272727] py-1 cursor-pointer font-medium' key={index}>{platform}</li>
                            ))
                        }
                    </ul>
                </div>
